...
lib.callback.register('acerp-mdt:server:getReports', function(source)
    local data = {}
    local results = MySQL.query.await('SELECT id, suspect, officer, title, text, warrant, date, deleted FROM mdt_reports')
    if results then
        for _, row in ipairs(results) do
            table.insert(data, {
...
RegisterNetEvent('acerp-mdt:server:addReport', function(sCID, oCID, title, text, warrant, date) 
    ...
    MySQL.insert.await('INSERT INTO `mdt_reports` (id, suspect, officer, title, text, warrant, date, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
        newId, sCID, oCID, title, text, warrant, formattedDate, 'false'
    })
end)
...
RegisterNetEvent('acerp-mdt:server:removeReport', function(id) 
    MySQL.update.await('UPDATE `mdt_reports` SET `deleted` = ? WHERE `id` = ?', {
        'true', id
    })
end)
